<template>
	<div class="">
		<svg class="h-4 w-4" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" x="0px" y="0px"
			viewBox="0 0 489.142 489.142" style="enable-background:new 0 0 489.142 489.142;" xml:space="preserve">
		<g>
			<path style="fill:#005ECE;" d="M489.142,460.858L364.614,336.329c-8.494,10.315-17.97,19.791-28.284,28.284l124.529,124.529
				L489.142,460.858z"/>
			<path style="fill:#9BFBFF;" d="M205.743,371.485V40C114.352,40,40,114.352,40,205.743S114.352,371.485,205.743,371.485z"/>
			<path style="fill:#46F8FF;" d="M371.485,205.743C371.485,114.352,297.133,40,205.743,40v331.485
				C297.133,371.485,371.485,297.133,371.485,205.743z"/>
			<path style="fill:#2488FF;" d="M364.614,336.329c29.272-35.548,46.872-81.05,46.872-130.587C411.485,92.296,319.189,0,205.743,0
				S0,92.296,0,205.743s92.296,205.743,205.743,205.743c49.537,0,95.039-17.6,130.587-46.872
				C346.644,356.12,356.12,346.644,364.614,336.329z M205.743,40c91.391,0,165.743,74.352,165.743,165.743
				s-74.352,165.743-165.743,165.743S40,297.133,40,205.743S114.352,40,205.743,40z"/>
		</g>
		</svg>
	</div>
</template>

<script>
export default {
  name: 'Search-svg'
}
</script>

<style scoped>
	.svg {
		border: none;
	}
</style>